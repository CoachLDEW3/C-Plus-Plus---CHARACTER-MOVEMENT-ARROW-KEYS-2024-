/************************************************************************************************
	...
	Purpose:	This is a simple program that demonstrates user input to move a character in the
				console using the arrow keys with GetAsyncKeyState(), with basic collision 
				detection to keep in within the borders (playing field)
*************************************************************************************************/
...
int main()
{
	...
	while (!gameOver)
	{
		...
		if (_kbhit())
		{
			...
			if (GetAsyncKeyState(VK_LEFT) && x > 3) x--;
			if (GetAsyncKeyState(VK_RIGHT) && x < 60) x++;
			if (GetAsyncKeyState(VK_UP) && y > 4) y--;
			if (GetAsyncKeyState(VK_DOWN) && y < 38) y++;
